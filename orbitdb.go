...
import (
	...
	ipfsAccess "github.com/stateless-minds/go-orbit-db/accesscontroller/ipfs"
	...
	acutils "github.com/stateless-minds/go-orbit-db/accesscontroller/utils"
	"github.com/stateless-minds/go-orbit-db/address"
	"github.com/stateless-minds/go-orbit-db/cache"
	"github.com/stateless-minds/go-orbit-db/cache/cacheleveldown"
...
var AtlasManifestEnvelope = atlas.BuildEntry(utils.ManifestEnvelope{}).
	StructMap().
	AddField("Manifest", atlas.StructMapEntry{SerialName: "manifest"}).
	...
	AddField("AccessController", atlas.StructMapEntry{SerialName: "access_controller"}). // Explicitly set CBOR key
	Complete()

func init() {
	cbornode.RegisterCborType(AtlasManifestEnvelope)
}
...
func NewOrbitDB(ctx context.Context, ipfs coreiface.CoreAPI, options *NewOrbitDBOptions) (iface.OrbitDB, error) {
	if ipfs == nil {
...
func newOrbitDB(ctx context.Context, is coreiface.CoreAPI, identity *idp.Identity, options *NewOrbitDBOptions) (iface.OrbitDB, error) {
	if is == nil {
		return nil, fmt.Errorf("ipfs is a required argument")
	}
	...
	orbit.RegisterStoreType("eventlog", eventlogstore.NewOrbitDBEventLogStore)
	orbit.RegisterStoreType("keyvalue", kvstore.NewOrbitDBKeyValue)
	orbit.RegisterStoreType("docstore", documentstore.NewOrbitDBDocumentStore)

	err = orbit.RegisterAccessControllerType(ipfsAccess.NewIPFSAccessController)
	if err != nil {
		return nil, err
	}
	err = orbit.RegisterAccessControllerType(orbitdb.NewOrbitDBAccessController)
	if err != nil {
		return nil, err
	}
	err = orbit.RegisterAccessControllerType(orbitdb_selfenroll.NewSelfEnrollAccessController)
	if err != nil {
		return nil, err
	}
	err = orbit.RegisterAccessControllerType(simple.NewSimpleAccessController)
	if err != nil {
		return nil, err
	}

	// set new heads as stateful, so newly subscriber can replay last event in case they missed it
	orbit.emitters.newHeads, err = options.EventBus.Emitter(new(EventExchangeHeads), eventbus.Stateful)
	if err != nil {
		return nil, fmt.Errorf("unable to create global emitter: %w", err)
	}

	if err := orbit.monitorDirectChannel(ctx, options.EventBus); err != nil {
...
func (o *orbitDB) Open(ctx context.Context, dbAddress string, options *CreateDBOptions) (Store, error) {
	...
	manifest := utils.ManifestEnvelope{}
	...
	store, err = o.createStore(ctx, *options.StoreType, parsedDBAddress, options)
	if err != nil {
		return nil, fmt.Errorf("unable to create store: %w", err)
	}
